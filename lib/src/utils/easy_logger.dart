...
class EasyLogger {
  ...
  void _onAdLoading(AdEvent event) {
    _logger.i(
        "${event.adUnitType?.value} ads for ${event.adNetwork.value} have been loading.");
  }
  ...
  void _onPaidEvent(AdEvent event) {
    final dataMap = event.data as Map<String, dynamic>?;
    _logger.i(
        "User has earned ${dataMap?['valueMicros']}/1000000 ${dataMap?['currencyCode']} from ${event.adNetwork.value}");
  }
